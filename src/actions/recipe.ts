...
export async function getRecipes() {
  try {
    ...
    return { success: false, error: "Помилка створення карточки" };
  }
}
...
export async function createRecipe(formData: FormData) {
  try {
    ...
    if (!name || ingredients.length === 0) {
      return {
        success: false,
        error: "Назва чи хочаб один параметр обов'язкові",
      };
    }
...
export async function deleteRecipe(id: string) {
  try {
    ...
    return { success: false, error: "Помилка видалення карточки" };
  }
}
